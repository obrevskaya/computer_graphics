...
void Canvas::close()
{
    if (!logic_handler.get_handler_state() || logic_handler.get_handler_state() == 2)
        return;

    if (logic_handler.get_handler_state() == 1)
        logic_handler.cutter_close();
    else if (logic_handler.get_handler_state() == 3)
        logic_handler.enter_close();
    process = not process;
    this->update();
}
...
{
    if (logic_handler.get_handler_state() != 1 && logic_handler.get_handler_state() != 3)
        process = not process;
