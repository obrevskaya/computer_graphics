...
void MainWindow::progInfo_messagebox() {
  QMessageBox::information(this, "ПРОГРАММА",
                           "Реализует алгоритм Сазерленда-Ходжмена отсечения "
                           "многоугольников выпуклым отсекателем");
}
...
void MainWindow::on_cutButton_clicked() {
  bool is_valid = ui->canvas->cut();
  if (not is_valid)
    error_messagebox("Некорректный отсекатель");
}
...
void MainWindow::on_closeButton_clicked() { ui->canvas->close(); }
