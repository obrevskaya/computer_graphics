...
polygones_t request::handle() {
  ...
  case REMOVE_POINT:
    ...
    break;
  case FILL:
    fill(polygones, params.color, params.sleep_data.is_sleep,
...
void request::fill(polygones_t &polygones, const color_t &color, bool is_sleep,
                   int sleep_time, std::vector<double> &time) {
  using std::chrono::duration_cast;
  using std::chrono::high_resolution_clock;
  using std::chrono::microseconds;
  for (size_t i = 0; i < polygones.size(); i++)
    if (!polygones[i].filled) {
      auto cur_time = high_resolution_clock::now();
      drawer.list_edges_flag_method(polygones[i],
                                    polygones[i].edges.back().p1.color, color,
                                    is_sleep, sleep_time);
